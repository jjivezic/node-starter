 ...
 * Provides methods for interacting with Google Gemini API
 */
class GeminiService {
  /**
  ...
  async chatWithHistory(messages, options = {}) {
    ...
    try {
      ...
      if (hasFunctionCall) {
        ...
        console.log('✅ Gemini requested tool calls (from candidates):', extractedCalls.map(fc => fc.name));
        return { toolCalls: extractedCalls };
      }
      
      if (functionCalls && functionCalls.length > 0) {
        console.log('✅ Gemini requested tool calls (from method):', functionCalls.map(fc => fc.name));
        return {
   ...
   */
  async createEmbedding(text, model = 'text-embedding-004') {
    try {
